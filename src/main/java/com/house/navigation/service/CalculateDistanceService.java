...
public class CalculateDistanceService {
    ...
    private MobileStationDTO getMobileStationDTO(UUID mobileStationUuid, List<ReportStation> reportStations, float errorRadius, double[] mobileStationCoordinates) {
        ...
        mobileStationDTO.setMobileStationUuid(mobileStationUuid);
        ...
        mobileStationDTO.setErrorRadius(errorRadius);
        if(reportStations.size() < 3) {
            mobileStationDTO.setErrorCode(301);
            mobileStationDTO.setErrorDescription("Number of Stations is less than 3, please wait for additional Base Report");
        } else {
            mobileStationDTO.setErrorCode(200);
            mobileStationDTO.setErrorDescription("3 Base Station Provided report. Calculation process is healthy");
        }
    ...
    private void saveLastMobileStationCoordinates(UUID mobileStationUuid, double[] mobileStationCoordinates) {
        ...
        mobileStation.setUuid(mobileStationUuid);
        mobileStation.setLastCoordinateX((float) mobileStationCoordinates[0]);
        mobileStation.setLastCoordinateY((float) mobileStationCoordinates[1]);
